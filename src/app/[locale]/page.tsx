...
export default function HomePage() {
  ...
  return <main className="home-page site-palette bg-[#dcecf0] text-[#102a43]">
    ...
    <section className="mx-auto grid max-w-7xl gap-12 px-5 py-24 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:px-12"><div className="relative min-h-[380px] overflow-hidden rounded-2xl bg-[#102a43]"><div className="absolute inset-0 bg-cover bg-center opacity-80" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=1200&q=85')" }} /><div className="absolute inset-0 bg-gradient-to-t from-[#061827] via-transparent to-transparent" /><div className="absolute bottom-6 start-6 flex items-center gap-3 text-sm font-bold text-white"><ShieldCheck className="h-5 w-5 text-cyan-300" /> {ar ? "توثيق قبل القرار" : "Evidence before decisions"}</div></div><div className="self-center"><p className="text-sm font-bold uppercase tracking-[0.14em] text-cyan-700">Dinoora / Case approach</p><h2 className="mt-4 text-4xl font-black tracking-tight">{text.caseTitle}</h2><p className="mt-5 text-lg leading-8 text-[#5d7180]">{text.caseIntro}</p><ul className="mt-8 grid gap-4 sm:grid-cols-2">{text.caseItems.map(item => <li key={item} className="flex gap-3 text-sm font-semibold text-[#385268]"><Check className="h-5 w-5 shrink-0 text-emerald-600" />{item}</li>)}</ul></div></section>
